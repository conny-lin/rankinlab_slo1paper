...
% 
% which comes from ?InitialEtohSensitivityPct.m?, 
%   which pulls data from ?DataTrv.mat? 
%   generated by Dance_ShaneSpark_v1707.m.
% 
% ~~~~~~~~
% Graph B 
% ~~~~~~~~
...
%   Data = Dance_rType2(pData,'pSave',pSave,'genotype',genotype);
%   which data comes from Dance_Raster
% 
% 
% Strategy:
...
for si = 1:numel(condStrains)
    ...
    ExpSets{si} = D;
    % export in csv
    p = sprintf('%s/2-Materials Methods',pPaper);
    pS = create_savefolder(p,'Experiment Info');
    filename = sprintf('%s/%s_experiment info.csv',pS,strainname);
...
%% Graph A
% extract data
for si = 1:numel(ExpSets)
    pMWT = ExpSets{si}.mwtpath;
    p = sprintf('3-Results/Data/%s',condStrains{si});
    pS = create_savefolder(pPaper,p);
    MWTSet = Dance_ShaneSpark_v5r1(pMWT,'pSave',pS);
end
...
for si = 1:numel(condStrains)
    pMWT = ExpSets{si}.mwtpath;
    strainname = condStrains{si};
    p = fullfile(pPaper, '3-Results','Data');
    pS = create_savefolder(p,strainname);
    MWTSet = Dance_Raster2(pMWT,'pSave',pS);
    Dance_rType2(pMWT,'pSave',pS);
