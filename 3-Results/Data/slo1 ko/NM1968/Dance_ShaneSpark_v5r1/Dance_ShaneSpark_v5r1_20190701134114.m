function MWTSet = Dance_ShaneSpark_v5r1(pMWT,varargin)
%% INFORMATION
% chor (added 20170606): built in chor
% v5r2: changed line 48, save DataTrv.mat doesn't matter what
...
[MWTSet,pSave] = DanceM_MWTSetStd3(pMWT,functionfilepath,Varin,...
    pSave,'timestamp',timestamp);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%% OBTAIN DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% chor 
[~,pMWTpass,pMWTfailed] = chormaster5('ShaneSpark',pMWT,'displayopt',displayopt); % chor
pMWTinput = pMWT;
pMWT = pMWTpass;
...
%% HAB CURVE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% stats %-----------------------------------------------------------------
Stats_HabCurveRMANOVA_v2(DataTrv,MWTDB,pSave);
%--------------------------------------------------------------------------
...
GT = {'','','tap'};
D = MWTSet.ByGroupPerPlate.(msrlist{1}).tap(:,1)';
for si  = 1:numel(statsnames)
    sname = statsnames{si};
    A = []; G = {}; M = {};
    for msri = 1:numel(msrlist)
        msr = msrlist{msri};
        A = [A; MWTSet.ByGroupPerPlate.(msr).(sname)'];
        G = [G; MWTSet.ByGroupPerPlate.(msr).groupname'];
        M = [M; repmat({msr},numel(MWTSet.ByGroupPerPlate.(msr).groupname),1)];

    end
    GT = [GT; 
            [M G repmat({sname},numel(G),1)]];
    D = [D;A];
end
T = table;
T.msr = GT(:,1);
...
T.stats = GT(:,3);
% create header
t = MWTSet.ByGroupPerPlate.(msrlist{1}).tap(:,1)';
ts = num2cellstr(t);
