function MWTSet = Dance_ShaneSpark4_Nlim8(pMWT,varargin)
%% INFORMATION
%% DEFAULTS
MWTSet = struct;
timestamp = 'off';
...
%% limit to N=8
D = MWTSet.Raw;
mwtid_lowN = unique(D.mwtid(D.Ntrack<8));
D(ismember(D.mwtid, mwtid_lowN),:) = [];

% see if still have all groups
MWTDB = MWTSet.MWTDB;
i = ismember(MWTDB.mwtid, unique(D.mwtid));
% tabulate(MWTDB.groupname(i))
MWTSet.Raw_NVal = D;

%% group eStat
DataTrv = MWTSet.Raw_NVal;
MWTDB = MWTSet.MWTDB;
MWTSet.ByGroupPerPlate = Stats_GroupByPlate(DataTrv,MWTDB,msrlist);
...
%% stats
Stats_HabCurveRMANOVA(MWTSet,pSave)
