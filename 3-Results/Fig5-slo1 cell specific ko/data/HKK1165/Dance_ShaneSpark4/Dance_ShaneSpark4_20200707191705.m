function MWTSet = Dance_ShaneSpark4(pMWT,pSave,varargin)
%% INFORMATION
% chor (added 20170606): built in chor

%% DEFAULTS
timestamp = 'off';
if nargin ==1; pSave = '/Users/connylin/Dropbox/RL/Dance Output'; end
Varin= varargin;
...
[MWTSet,pSave] = DanceM_MWTSetStd3(pMWT,mfilename('fullpath'),Varin,...
...
%% HABITUATION CURVES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% stats
Stats_HabCurveRMANOVA(MWTSet.Raw ,MWTSet.MWTDB, pSave);
% group eStat ------------------------------------------------------------
...
%% SAVE AND FINALIZED  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
save(fullfile(pSave,[MWTSet.ProgramName,'.mat']),'MWTSet');
% Report done
fprintf('\n\n***DONE***\n\n');
return
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mwtidr = MWTSet.MWTDB.mwtid;
mwtidd = Data.mwtid;
[i,j] = ismember(mwtidd,mwtidr);
groups = MWTSet.MWTDB.groupname(j(i));
Data.groups = groups;
exp = MWTSet.MWTDB.expname(j(i));
Data.expname = exp;

for x = 1:numel(msrlist)
    msr = msrlist{x}; % get measure
    d = Data.(msr); % get data
    [txt,anovastats,multstats,T] = anova1_std_v2(d,groups,'plate');
    anovastats.pvalue = multstats.pValue;
    anovastats.descriptive = T;

    MWTSet.Stats.(msr).(component) = anovastats;
    % text output
    fid = fopen(fullfile(pSave,sprintf('ANOVA %s.txt',component)),'w');
    fprintf(fid,'%s',txt);
    fclose(fid);
    % table output
    writetable(T,fullfile(pSave,sprintf('Descriptive %s.csv',component)));
end
...
%% PERCENT HAB COMPARED TO CONTROL %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%% STATISTICS FOR GRAPHS (by group by plate)
% D = MWTSet.ByGroupPerPlate;
% gnames = fieldnames(D);
% B = struct;
% B.GroupNames = gnames;
...
for m = 1:numel(M) % for each measure
    % write to table (20190804)
    T = saveGraphstruct2table(Graph.(M{m}),fullefile(pSaveA,[msr,'.csv']));
