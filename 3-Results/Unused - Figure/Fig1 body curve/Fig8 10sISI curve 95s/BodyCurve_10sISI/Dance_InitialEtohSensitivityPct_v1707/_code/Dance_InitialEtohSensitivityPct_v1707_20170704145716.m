...
%% PCT STATS 
msrlist = {'curve','speed'};
for msri = 1:numel(mrslist)
    msr = msrlist{msri};
    
    CS = CurveStats_v1707;
    CS.data = DataMeta.(msr);
    CS.MWTDB = MWTDB;
    CS.mwtid = Data.Meta.mwtid;
    MWTSet.Stats.(msr) = CS;

    % write table
    A = CS.dscByGroup;
    writetable(A,fullfile(pSave,sprintf('%s.csv',msr)));

    A = CS.pctByPlate;
    g = A.groupname;
    x = A.pct;
    T = statsBasicG(x,g);
    writetable(A,fullfile(pSave,sprintf('%s pct.csv',msr)));


    fid = fopen(fullfile(pSave,sprintf('%s pct.txt',msr)),'w');
