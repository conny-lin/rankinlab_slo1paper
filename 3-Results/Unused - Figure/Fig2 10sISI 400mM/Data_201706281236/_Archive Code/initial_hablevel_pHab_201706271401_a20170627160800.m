...
i = ismember(Data.groups,ctrl);
Data_Ctrl = Data(i,:);
TN = tabulate(Data_Ctrl.expname);
n = cell2mat(TN(:,2));
expV = TN(n>1,1);
...
for msri = 1:numel(msrlist)
    msr = msrlist{msri};
    DCT = statsBasicG(Data_Ctrl.(msr),Data_Ctrl.expname,'expname');
    dc = DCT.mean;
    [i,j] = ismember(Data_Exp.expname, DCT.expname);
    dcontrol = DCT.mean(j(i));
    dexp = Data_Exp.(msr);
    dpct = dexp./dcontrol;
    Data_PCT.(msr) = dpct;

    % stats
    stats = statsBasic(dpct,'outputtype','struct');
    [text,p,ci,s] = ttest_auto(dpct,1);
    stats.pvalue = p;
    stats.ttest = text;
    ...
    fprintf(fid,'%s, %s\n',msr,text);
    % table output
    T = statsBasic(dpct,'outputtype','table');
    T1 = table;
    T1.msr = {msr};
    T1.N = T.N;
    T1.mean = T.mean;
...
% table output
writetable(T,fullfile(pSave,sprintf('Descriptive %s pct %s.csv',component,msr)));
