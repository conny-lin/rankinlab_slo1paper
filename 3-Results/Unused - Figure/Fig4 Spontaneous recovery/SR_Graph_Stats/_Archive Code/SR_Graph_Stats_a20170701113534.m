...
SUM = table;
MSR = {};
pSaveD = create_savefolder(pSave,'RawData');
fid = fopen(fullfile(pSave,'Stats.txt'),'w');
for msri = 1:numel(msrlist) % cycle by measures
    ...
    for x = 1:numel(ISI) % cycle through isi
        for gi = 1:numel(grouplist) % cycle through groups
            ...
            fn = sprintf('%s_Mean %s.csv',msr, gn); % file name
            str = fullfile(pHome, foldername, 'Dance_Glee_Acafellas','Data Raw',fn); % get path to raw data
            copyfile(str,fullfile(pSaveD,sprintf('%s_Mean %s %dsISI.csv',msr, gn,isi))); % copy file
    ...
    %% anovan
    var = {'ISI','group'};
    [sanova,S1] = anovan_std(T.y, {T.ISI T.group}, var, pSave, 'prefix','','suffix',msr,'export',{'text'});
    
    
    % add to summary table
    a = repmat({msr},size(S1,1),1);
    MSR = [MSR;a];
    SUM = [SUM;S1];
...
%% summarize table
g = regexpcellout(SUM.gnameu,' ','split');
T = table;
T.ISI = g(:,1);
T.groupname = regexprep(g(:,2),'_',' ');
T.msr = MSR;
SUM= [T SUM];
% save table
[p,fn] = fileparts(pSave);
writetable(SUM,fullfile(pSave,sprintf('%s.csv',fn)));
% report done
fprintf(' ~~~ Done ~~~\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
