...
for gi = 1:numel(glist) % cycle through groups
    ...
    for ti = 1:numel(timelist) % cycle through time choices
        ...
        % time info -----------------
        matfiletime1now = timelist(ti);
        timestartnow = matfiletime1now + 5;
...
%         tfilename = ['data_ephys_',tname,'.mat'];
        ...
        j = find(rTime >= timestartnow & rTime <= timeendnow);
        i = j(1);
        % get data before tap Baseline
        baselinetimei = i-3:1:i-1;
        baselinetime = rTime(baselinetimei);
        responsetimei = i:1:i+5;
        responsetime = rTime(responsetimei);
        Baseline = Data(:,baselinetimei);
        Response = Data(:,responsetimei); % 1s after the tap
        % ----------------
return
