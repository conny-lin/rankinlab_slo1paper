...
% construct times
taptimelist = [tapstarttime:isi:(tapstarttime+isi*tapN)];
...
for gi = 1:numel(glist) % cycle through groups
    ...
    for ti = 1:numel(matfiletimelist) % cycle through time choices
        ...
        % report progress %------------
        fprintf(''); % separator
        processIntervalReporter(numel(matfiletimelist),1,'time',ti);
        % ------------------------------
...
return
        ...
        % create output array
        DataStats = cell(size(responseTypes),1);
        ...
        for ri = 1:numel(responseTypes)
            ...
            % transform data
            R = reshape(RespNv,numel(RespNv),1);
            times = repmat(t,size(RespNv,1),1);
            DataStats{ri} = [R times];
...
matfiletimelist = 100:10:(100+(29*10));
coli = [1:6:(30*6)];
% --------------------------------
...
for gi = 1:numel(glist)
    ...
    y = d.mean(1:2,coli)';
    e = d.se(1:2,coli)';
    x = repmat(matfiletimelist,size(d.mean,1)-1,1)';
    % ------------------------------------------
