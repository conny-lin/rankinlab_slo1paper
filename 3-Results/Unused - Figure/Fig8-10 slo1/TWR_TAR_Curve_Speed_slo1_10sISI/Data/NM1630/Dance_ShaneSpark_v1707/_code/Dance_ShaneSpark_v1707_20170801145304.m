function MWTSet = Dance_ShaneSpark_v1707(pMWT,pSave,varargin)
%% INFORMATION
...
pctstats = true;
% set data extraction type
% if for Trv only, set to 'Trv'
dataExtract = 'Reversal'; % for reversal
displayopt = true;
VarIn = varargin;
%% varargin processor
...
[MWTSet,pSave] = DanceM_MWTSetStd_v1707(pMWT,funpath,VarIn,pSave);
[Legend,pMWTpass,pMWTfailed] = chormaster5('ShaneSpark',pMWT,'displayopt',displayopt); % chor
[MWTSet,pMWT] = DanceM_MWTSet_postChor(MWTSet,pMWT,pMWTpass,pMWTfailed,Legend);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%% OBTAIN DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
MWTSet.Raw = import_trv_table(pMWT,'displayopt',displayopt); % get trv data
% if only extracting Trv data
if strcmp(dataExtract,'Trv')
    save(fullfile(pSave,'DataTrv.mat'),'MWTDB','DataTrv');
    return
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% HAB CURVE --------------------------------------------------------------
% Stats_HabCurveRMANOVA_v2(MWTSet.Raw,MWTSet.MWTDB,pSave); % stats
[MWTSet,~,~] = Stats_HabCurveRMANOVA_v1707(MWTSet,pSave);
MWTSet = Graph_HabCurve_v1707(MWTSet,msrlist); % graphing
%--------------------------------------------------------------------------
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% get data together
component = 'Initial';
MWTDBG = MWTDB(:,{'mwtid','groupname','expname'}); % get group info together
Data = MWTSet.Raw(MWTSet.Raw.tap==1,:);
Data = innerjoin(MWTDBG,Data);
for msri = 1:numel(msrlist)
    msr = msrlist{msri}; % get measure
    D = Data(:,{'mwtid','groupname','expname',msr});
    D(D.(msr) == 0,:) = [];  % exclude initial = 0
    MWTSet.Stats.(msr).(component).data = D;
end
% anova
MWTSet = Stats_Output_ANOVA_GraphTable(Data,MWTSet,msrlist,component,pSave);
% within group percent initial
if pctstats
    MWTSet = Stats_Output_ttest_pctCtrl(MWTSet,msrlist,Data,ctrl,component,pSave,1);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
component = 'Hablevel';
MWTDBG = MWTDB(:,{'mwtid','groupname','expname'}); % get group info together
Data = MWTSet.Raw(ismember(MWTSet.Raw.tap,28:30),:); % get data
% calculate hab level
Data1 = MWTDBG;
for msri = 1:numel(msrlist)
    msr = msrlist{msri}; % get measure
    A = statsBasicG(Data.(msr),Data.mwtid,'mwtid');
    D = innerjoin(MWTDBG,A);
    MWTSet.Stats.(msr).(component).data = D;
    B = table;
    B.mwtid = A.mwtid;
    B.(msr) = A.mean;
    Data1 = innerjoin(Data1,B);
end
Data = Data1; 
clear Data1 A D msr msri;
% anova
MWTSet = Stats_Output_ANOVA_GraphTable(Data,MWTSet,msrlist,component,pSave);
% within group percent initial
if pctstats
...
for msri = 1:numel(msrlist)
    msr = msrlist{msri}; % get measure
    % get data
    D1 = MWTSet.Stats.(msr).Initial.data;
    D1.Properties.VariableNames(ismember(D1.Properties.VariableNames,msr)) = {'Initial'};
    Df = MWTSet.Stats.(msr).Hablevel.data;
    Df.Properties.VariableNames(ismember(Df.Properties.VariableNames,'mean')) = {'HabLevel'};
    Df(:,{'n','se'}) = [];
    D = D1(:,{'mwtid','groupname','expname'});
    D = innerjoin(D,D1);
    D = innerjoin(D,Df);
    hb = D.HabLevel;
    ini = D.Initial;
    ...
    MWTSet.Stats.(msr).databyplate_component = D;
    MWTSet.Stats.(msr).(component).data = D(:,{'mwtid','groupname','expname',component});

    B = table;
    B.mwtid = D.mwtid;
    B.(msr) = D.(component);
    Data = innerjoin(Data,B);
end
% anova
MWTSet = Stats_Output_ANOVA_GraphTable(Data,MWTSet,msrlist,component,pSave);
% within group percent initial
