...
for msri = 1:numel(msrlist)
    ...
    rmTerm = sprintf('%s1-%s13~%s',rmName,rmName,gFactorName);
    ...
    compName = 'gname';
    rm = fitrm(T,'t1-t13~gname','WithinDesign',withinDesignT);
    ...
    % comparison by group each time +++++++++++
    compName = 'gname';
    t = multcompare(rm,rmName,'By',compName);
    %  keep only unique comparisons
    a = strjoinrows([t.([rmName,'_1']) t.([rmName,'_2'])],' x ');
    save(fullfile(pM,[msr ,' by group per time.mat']),'t');
%     t(~ismember(a,gpairs),:) =[];
    t(t.pValue > alpha,:) = [];
    % record
    textout = sprintf('%s\n\nPosthoc(Tukey)%s by %s:\n',textout,compName,rmName); 
    if isempty(t); 
        textout = sprintf('%s\nAll comparison = n.s.\n',textout);
    else
        t = multcompare_text2016b(t,'pvlimit',pvlimit,'alpha',alpha);
