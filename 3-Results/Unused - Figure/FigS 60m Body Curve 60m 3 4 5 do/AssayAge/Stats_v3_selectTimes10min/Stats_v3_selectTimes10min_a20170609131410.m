...
for msri = 1:numel(msrlist)
    ...
    for gi = 1:numel(groupnames)
        ...
        [anovatext,phtext,Tanova] = anova1_std(data,gid);
    ...
    compName = 'gname';
    t = multcompare(rm,compName,'By',rmName);
    savename_suffix = 'by time within group';
    %  keep only unique comparisons
    a = strjoinrows([t.([compName,'_1']) t.([compName,'_2'])],' x ');
    savename = fullfile(pM,[msr ,' ',savename_suffix,'.mat']); % create save path
    save(savename,'t'); % save
    % create text output
    t(~ismember(a,gpairs),:) =[]; % delete repeated results
    t(t.pValue > alpha,:) = []; % delete insignificant results
    textout = sprintf('%s\n\nPosthoc(Tukey)%s by %s:\n',textout,rmName,compName); 
    if isempty(t)
        textout = sprintf('%s\nAll comparison = n.s.\n',textout);
    else
