...
for msri = 1:numel(msrlist)
    ...
    for gi = 1:numel(groupnames)
        ...
        fname = sprintf('%s btw time within group pvalue Matrix %s.csv',msr, gn);
        csvwrite(fullfile(pM,fname),M)
...
A.datapath = sprintf('%s %s.txt',msr,suffix);
A.txt_t_identifier = '*** N2_200mM ***'
