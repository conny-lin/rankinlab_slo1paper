...
for msri = 1:numel(msrlist)
    ...
    for gi = 1:numel(groupnames)
        ...
        gid(2:end) = (gid(2:end)-1)*incr; % update to actual time
        ...
        [txt,anovastats,multstats] = anova1_std_v2(data,gid,'plate','digit',2,'ns',0);
        % central txt output
        fprintf(fid,'#### %s #####',gn); % add header
        fprintf(fid,'%s\n',txt); % add to printout
        ...
        r = multcompare_text2016b(multstats); 
        fprintf(fid,'*** comparison between times within %s ***\n',gn); % add header
        fprintf(fid,'%s\n',r); % add to printout
        fprintf(fid,'#############\n\n');
