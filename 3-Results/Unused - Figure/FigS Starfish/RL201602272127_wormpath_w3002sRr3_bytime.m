...
for ti = 1:size(timeAssay,2)
    ...
    for randrepeat = 1:nrepeat
        ...
        for gi = 1:numel(gnameu)
            gn = gnameu{gi};
            fprintf('rand#%d/%d: %s %.0fs-%.0fs\n',randrepeat,nrepeat,gn,assayTime1,assayTime2);
            ...
            % figure
            if assayTime2 < 100;
                fig1 = starFish(wormid,X,Y,Bias,Tap,'visibleopt',0,'tapcirclesize',10,'pathlinewidth',pathlinewidth,'wormidshow',0,'alignAtTap',1);
            else
                fig1 = starFish(wormid,X,Y,Bias,Tap,'respondantsOnly',1,'visibleopt',0,'tapcirclesize',10,'pathlinewidth',pathlinewidth,'wormidshow',0,'alignAtTap',1);
            end
            % label and save
%             tname = sprintf('%s %.1fs-%.1fs N(worm)=%d',regexprep(gn,'_','-'),assayTime1,assayTime2,nwormselect);
%             title(tname)
            savename = sprintf('starfish %s %.0f-%.0fsrandomN%dx%d',regexprep(gn,'_','-'),assayTime1,assayTime2,nwormselect,randrepeat);
